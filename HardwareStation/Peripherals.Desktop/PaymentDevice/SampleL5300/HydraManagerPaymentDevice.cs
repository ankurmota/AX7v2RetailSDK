...
namespace Contoso
{
    namespace Commerce.HardwareStation.Peripherals.SampleL5300
    {
        using System;
        ...
        using Commerce.HardwareStation.Peripherals.PaymentTerminal.SampleL5300;
        ...
        using LocalPaymentTerminal = PaymentTerminal;

        /// <summary>
        /// HydraManagerPaymentDevice class.
        /// </summary>
        [Export("HYDRA", typeof(IPaymentDevice))]
        public class HydraManagerPaymentDevice : IPaymentDevice, IDisposable
        {
            ...
            private HydraPaymentDevice paymentDevice;
            ...
            public async Task BeginTransactionAsync(PaymentProperty[] merchantProperties, string paymentConnectorName, string invoiceNumber, bool isTestMode)
            {
                ...
                {
                    await this.paymentDevice.BeginTransactionAsync(null);
                    this.paymentDevice.CardSwipeEvent += (sender, args) => this.cardTrackData = new LocalPaymentTerminal.TrackData { Track1 = args.Track1, Track2 = args.Track2 };
                    await this.paymentDevice.SetCardSwipeAsync();
                });
            }
            ...
            public async Task EndTransactionAsync()
            {
                await this.Execute(async () =>
                {
                    await this.paymentDevice.ShowFormAsync(Form.ThankYou, null);
            ...
            public async Task UpdateLineItemsAsync(string totalAmount, string taxAmount, string discountAmount, string subTotalAmount, IEnumerable<ItemInfo> items)
            {
                ...
                await this.Execute(async () =>
                {
                    await this.paymentDevice.ShowFormAsync(Form.Total, formProperties);
                });
            }
            ...
            public async Task<string> RequestTenderApprovalAsync(decimal amount)
            {
                return await this.Execute(async () =>
                {
                    ...
                    await this.paymentDevice.ShowFormAsync(Form.Processing, formProperties);
            ...
            private async Task<string> GetSignatureData()
            {
                ...
                this.paymentDevice.SignatureEvent += signatureHandler;
                await this.paymentDevice.ShowFormAsync(Form.Signature, null);
                await this.paymentDevice.SetSignatureCaptureAsync();
            ...
            private async Task<TenderInfo> FillTenderInfo(bool maskCardNumber, bool allowCashback)
            {
                ...
                if (string.IsNullOrEmpty(this.tenderInfo.TenderId) || this.tenderInfo.TenderId.Equals(CancelButton, StringComparison.OrdinalIgnoreCase))
                {
                    // User aborted
                    return null;
                }
            ...
            private async Task<string> GetButtonPress(string formName)
            {
                ...
                // Some device don't forms, if false is returned skip the waiting for button to be pressed.
                bool isOkay = await this.paymentDevice.ShowFormAsync(formName, null);
                string button = string.Empty;

                if (isOkay)
                {
                    button = await getButtonPress.Task;
                }
            ...
            private async Task<string> GetDebitCardEncryptedPin(string cardNumber)
            {
                ...
                await this.paymentDevice.SetPinPadAsync(cardNumber);
            ...
            private async Task<LocalPaymentTerminal.TrackData> GetCardSwipeData()
            {
                ...
                this.paymentDevice.CardSwipeEvent += tenderHandler;
                await this.paymentDevice.ShowFormAsync(Form.CardSwipe, null);
                await this.paymentDevice.SetCardSwipeAsync(); // NOTE: This is not required for the L5300 HydraPaymentDevice as the Form controls swipe enable
