...
module Commerce.Operations {
    ...
    export class GetLatestNumberSequenceOperation extends DeviceActivationOperationBase {
        ...
        public operationName(): string {
            return Helpers.DeviceActivationHelper.GET_LATEST_NUMBER_SEQUENCE_OPERATION_NAME;
        }
        ...
        public processingStatusName(): string {
            return ViewModelAdapter.getResourceString("string_8051");
        }
        ...
        public errorStatusName(): string {
            return ViewModelAdapter.getResourceString("string_8052");
        }
        ...
        public operationProcess(): () => IVoidAsyncResult {
            return () => {
                ...
                var authenticationManager: Model.Managers.IAuthenticationManager =
                    Model.Managers.Factory.getManager<Model.Managers.IAuthenticationManager>(Model.Managers.IAuthenticationManagerName);

                return new AsyncQueue().enqueue((): IVoidAsyncResult => {
                    return channelManager.getLatestNumberSequence()
                        .done((numberSequences: Model.Entities.NumberSequenceSeedData[]) => {
                            NumberSequence.Update(numberSequences);
                        });
                }).enqueue((): IVoidAsyncResult => {
                    return authenticationManager.startSessionAsync();
                }).run();
            };
        }
    }
